...
export const Authprovider=({ children })=>{
 ...
 const [Server_Url]=useState("http://localhost:4000/api/")
...
async function checkAuth() {
  try {
    const resp= await axios.get("http://localhost:4000/api/auth/checkAuth",{withCredentials:true});
...
async function logout() {
    try {
      const resp =await axios.get("http://localhost:4000/api/auth/logout", {withCredentials:true});
